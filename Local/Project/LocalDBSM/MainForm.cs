...
namespace LocalDBSM
{
    public partial class MainForm : Form
    {
        ...
        private void sortTableButton_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (DataGridViewColumn column in valuesGrid.Columns)
                {
                    if (column.Selected)
                    {
                        string columnName = column.Name;
                        DBController.SortTable(selectedTableId, columnName);
                    }
                }
            }
            catch (Exception ex)
